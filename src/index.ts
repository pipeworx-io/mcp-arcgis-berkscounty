 ...
 *
 * Berks County GIS (ArcGIS org dGYe1jDYrTw1wwpc): parcels, addresses, zoning & planning. search_datasets finds layers (each
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Arcgis Berkscounty');
}


const HUB = 'https://hub.arcgis.com/api/v3/datasets';
const UA = 'pipeworx-mcp-arcgis-berkscounty/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Berks County GIS open geospatial datasets (parcels, addresses, zoning & planning) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (Berks County GIS).' },
